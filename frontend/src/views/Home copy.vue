   ...
   <div class="py-4 flex flex-col h-screen">
      ...
      <nav class="border-b pb-2 fixed top-0 w-full bg-white">
         ...
         <main class="flex w-full items-center justify-between container max-w-7xl">
            ...
            <router-link class="flex items-center gap-4 lg:flex-1 lg:px-12" :to="`/student/study-ue`">
            ...
            <div class="flex gap-8">
               ...
               <div v-if="user?.admin && uncorrectedCount > 0" class="cursor-pointer" @click="goToAdminCorrections">
                  ⏰
               </div>
...
onMounted(async () => {
   ...
   if (user_.admin) {
      ...
      // router.push(`/student/welcome-student`)
   }
})
...
const goToMessages = () => router.push(user.value.admin ? `/admin/admin-messages` : `/student/messages`)
