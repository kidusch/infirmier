<template>
   ...
   <ul v-for="topic, index in topicList">
      ...
      <EditableListItem field="name" :index="index" :list="topicList" @update="updateList" @edit="(text) => edit(topic.id, text)" @remove="remove(topic.id)" @select="select(topic.id)"></EditableListItem>
   </ul>
